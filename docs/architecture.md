...
6. 検索時はHaversine公式で直線距離を計算し、スコアと組み合わせてランキング表示
...
### 距離計算（Haversine）
- 外部サービス不要、appコンテナ内で完結
- 自宅座標と釣り場の緯度経度からHaversine公式で直線距離（km）を計算
- ADR 0005 参照
